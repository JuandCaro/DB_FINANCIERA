...
(
    id_depto       		  VARCHAR                       NOT NULL    CHECK   (LENGTH(id_depto)=2),
    nom_depto      		  VARCHAR                       NOT NULL    CHECK   (LENGTH(nom_depto) <=30),
...
(
    id_inversor           VARCHAR                       NOT NULL    CHECK   (LENGTH (id_inversor) =10),
    id_ciudad             VARCHAR                       NOT NULL    CHECK   (LENGTH (id_ciudad) =5),
...
(
    ...
    nom_presupuesto       VARCHAR                       NOT NULL     CHECK  (LENGTH (nom_presupuesto) <255),
    monto_presupuesto     DECIMAL   (8,0)               NOT NULL     CHECK  (monto_presupuesto >= 0),
    fec_inicio            DATE                          NOT NULL     CHECK  (fec_inicio >  '2025/01/1' 	    AND   fec_inicio < '2025/12/31'),
    fec_fin               DATE                          NOT NULL     CHECK  (fec_fin    <  '2025/12/31' 	AND   fec_fin	 > '2025/01/1'),
...
(
    ...
    id_inversor         	VARCHAR                       NOT NULL      CHECK  (LENGTH (id_inversor) =10),
    id_presupuesto      	VARCHAR                       NOT NULL      CHECK  (LENGTH (id_presupuesto) >=1     	AND LENGTH (id_presupuesto) <=10),
    nom_proyecto	        VARCHAR                       NOT NULL      CHECK  (LENGTH (nom_proyecto) <255),
    descripcion	            TEXT                          NOT NULL,     --CHECK  (),
    fec_inicio              DATE                          NOT NULL      CHECK  (fec_inicio >  '2025/01/1' 	    AND   fec_inicio < '2025/12/31'),
    fec_fin                 DATE                          NOT NULL      CHECK  (fec_fin    <  '2025/12/31' 	    AND   fec_fin	 > '2025/01/1'),
    presupuesto_asignado	DECIMAL(8,0)                  NOT NULL      CHECK  (presupuesto_asignado BETWEEN 0  AND  99999999),
...
(
    ...
    nom_activo	            VARCHAR                       NOT NULL       CHECK (LENGTH(nom_activo) <255),
    ...
    ubicacion	            VARCHAR                       NOT NULL       CHECK (LENGTH (ubicacion) <=100),
    datos_audit           AUDIT_TRAIL,
...
(
    ...
    id_activo	            VARCHAR                       NOT NULL       CHECK (LENGTH (id_activo) >=0 AND LENGTH (id_activo) <=100),
    descripcion	            TEXT                          NOT NULL,
    tipo	                VARCHAR                       NOT NULL       CHECK (tipo IN ('Rentabilidad','Liquidez','Endeudamiento','Rotación')),
    formula	                VARCHAR                       NOT NULL       CHECK (LENGTH(formula) <=255),
    lim_alerta	            DECIMAL(2,0)                  NOT NULL       CHECK (lim_alerta BETWEEN 0 AND 99),  
    datos_audit           AUDIT_TRAIL,
    PRIMARY KEY (id_ind),
...
(
    id_tercero	            VARCHAR                        NOT NULL       CHECK (LENGTH (id_tercero) =10),
